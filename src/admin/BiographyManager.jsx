...
import { 
  Save, UserCircle, Plus, Trash2, ChevronDown, ChevronUp, 
  Globe, Languages, Info, Layout, Layers
} from 'lucide-react';
...
const DEFAULT_BIO_SECTIONS = [
 ...
 (2010 سے آج تک)
  ...
  ممبرالقلم ایجوکیشنل AND ویلفیئر سوسائٹی (این جی او)، لاہور
...
const BiographyManager = () => {
  ...
  const addSection = () => {
    setSections([...sections, { title: "New Section", icon: "📄", en: "New Section", urduContent: "", enContent: "" }]);
  ...
  const removeSection = (index) => {
    if (window.confirm('Remove this entire section?')) {
      ...
      setSections(newSections);
      if (expandedIndex >= newSections.length) setExpandedIndex(Math.max(0, newSections.length - 1));
    }
  };

  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh' }}>
      <div style={{ color: 'var(--admin-accent)', fontWeight: '700' }}>Loading Structured Biography...</div>
    </div>
  );

  return (
    <div style={{ animation: 'fadeUp 0.5s ease', maxWidth: '1100px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '40px' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: 'var(--admin-accent)', marginBottom: '12px' }}>
             <Layers size={32} />
             <h2 style={{ fontSize: '32px', fontWeight: '800', color: 'var(--admin-text-main)', margin: 0 }}>Professional Biography</h2>
          </div>
          <p style={{ color: 'var(--admin-text-light)', fontSize: '15px' }}>
             Manage the structured life history sections shown on the public website.
          </p>
        </div>
        <button onClick={handleSave} className="btn-primary" style={{ padding: '14px 40px', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <Save size={20} /> Update All Sections
        </button>
      </div>
      ...
      {saveStatus && (
        <div style={{ 
          ...
          fontWeight: '700', border: '1px solid currentColor', display: 'flex', alignItems: 'center', gap: '10px'
        }}>
           <Info size={18} /> {saveStatus}
        </div>
      )}
      ...
      <div style={{ display: 'grid', gap: '20px' }}>
          ...
          <div key={idx} className="card" style={{ overflow: 'hidden', borderLeft: expandedIndex === idx ? '6px solid var(--admin-accent)' : '1px solid var(--admin-border)' }}>
            <div 
              ...
              style={{ 
                ...
                background: expandedIndex === idx ? 'var(--admin-bg)' : '#fff' 
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                <span style={{ fontSize: '24px' }}>{section.icon}</span>
                <div>
                   <div style={{ fontSize: '18px', fontWeight: '800', color: 'var(--admin-text-main)' }} className="urdu">{section.title}</div>
                   <div style={{ fontSize: '12px', color: 'var(--admin-text-light)', fontWeight: '700', textTransform: 'uppercase' }}>{section.en}</div>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                 <div style={{ display: 'flex', gap: '4px' }}>
                    <button onClick={(e) => { e.stopPropagation(); moveSection(idx, -1); }} disabled={idx === 0} style={{ padding: '6px', borderRadius: '8px', border: '1px solid var(--admin-border)', background: '#fff', cursor: 'pointer' }}><ChevronUp size={16} /></button>
                    <button onClick={(e) => { e.stopPropagation(); moveSection(idx, 1); }} disabled={idx === sections.length - 1} style={{ padding: '6px', borderRadius: '8px', border: '1px solid var(--admin-border)', background: '#fff', cursor: 'pointer' }}><ChevronDown size={16} /></button>
                 </div>
                 <button onClick={(e) => { e.stopPropagation(); removeSection(idx); }} style={{ padding: '8px', borderRadius: '8px', border: 'none', background: '#fef2f2', color: 'var(--admin-danger)', cursor: 'pointer' }}><Trash2 size={18} /></button>
              </div>
            </div>
              ...
              <div style={{ padding: '40px', background: '#fff', borderTop: '1px solid var(--admin-border)' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '32px', marginBottom: '32px' }}>
                   <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                      <div>
                        <label style={{ display: 'block', fontSize: '12px', fontWeight: '800', color: 'var(--admin-text-light)', marginBottom: '8px' }}>SECTION ICON</label>
                        <input type="text" value={section.icon} onChange={(e) => updateSection(idx, 'icon', e.target.value)} style={{ width: '60px', textAlign: 'center', padding: '12px', borderRadius: '10px', border: '1px solid var(--admin-border)', fontSize: '20px' }} />
                      </div>
                      <div>
                        <label style={{ display: 'block', fontSize: '12px', fontWeight: '800', color: 'var(--admin-text-light)', marginBottom: '8px' }}>URDU TITLE</label>
                        <input type="text" value={section.title} onChange={(e) => updateSection(idx, 'title', e.target.value)} className="urdu" style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid var(--admin-border)', fontSize: '18px' }} />
                      </div>
                      <div>
                        <label style={{ display: 'block', fontSize: '12px', fontWeight: '800', color: 'var(--admin-text-light)', marginBottom: '8px' }}>ENGLISH TITLE</label>
                        <input type="text" value={section.en} onChange={(e) => updateSection(idx, 'en', e.target.value)} style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid var(--admin-border)', fontSize: '14px', fontWeight: '700' }} />
                      </div>
                   </div>
                   ...
                   <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                           <Languages size={14} color="var(--admin-accent)" />
                           <label style={{ fontSize: '12px', fontWeight: '800', color: 'var(--admin-text-main)' }}>URDU CONTENT</label>
                        </div>
                        <div style={{ borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--admin-border)', background: '#fff' }}>
                          <RichTextEditor 
                            value={section.urduContent} 
                            onChange={(val) => updateSection(idx, 'urduContent', val)} 
                            lang="urdu"
                            style={{ height: '280px', marginBottom: '40px', fontSize: '17px', lineHeight: '1.8' }} 
                          />
                        </div>
                      </div>
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                           <Globe size={14} color="var(--admin-text-light)" />
                           <label style={{ fontSize: '12px', fontWeight: '800', color: 'var(--admin-text-main)' }}>ENGLISH TRANSLATION</label>
                        </div>
                        <div style={{ borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--admin-border)', background: '#fff' }}>
                          <RichTextEditor 
                            value={section.enContent} 
                            onChange={(val) => updateSection(idx, 'enContent', val)} 
                            style={{ height: '180px', marginBottom: '40px', fontSize: '14px' }} 
                          />
                        </div>
                      </div>
                   </div>
        ...
        <button 
          onClick={addSection}
          style={{ 
            padding: '24px', borderRadius: '20px', border: '2px dashed var(--admin-border)', 
            background: 'transparent', color: 'var(--admin-text-light)', fontWeight: '700',
          ...
          onMouseOut={(e) => { e.currentTarget.style.borderColor = 'var(--admin-border)'; e.currentTarget.style.color = 'var(--admin-text-light)'; }}
        >
          <Plus size={24} /> Add New Biography Section
        </button>
      </div>

      <div style={{ marginTop: '64px', padding: '40px', borderRadius: '24px', background: 'var(--admin-sidebar)', color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
         <div>
            <h3 style={{ fontSize: '20px', fontWeight: '800', marginBottom: '8px' }}>Ready to publish?</h3>
            <p style={{ color: 'rgba(255,255,255,0.6)', margin: 0, fontSize: '14px' }}>All changes will be instantly updated on your public website biography.</p>
         </div>
         <button onClick={handleSave} className="btn-primary" style={{ padding: '16px 48px', fontSize: '18px' }}>
           Publish Updates
         </button>
      </div>
    </div>
